...
.PHONY: test-python
# Note: the `ray` extra is intentionally NOT installed here. Its dependencies (ray, pandas,
# numpy) are heavyweight and numpy's installed stubs are incompatible with our mypy
# `python_version` target. The ray integration is exercised separately by the
# `test-python-ray` target.
test-python:
	uv --directory python/foxglove-sdk lock --check
...
# dependencies are heavy and pull in numpy stubs that break our mypy config (so mypy is not
# run here). test_ray.py skips its ray-dependent cases when the extra is absent.
test-python-ray:
