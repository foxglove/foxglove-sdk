...
- **suppress_video_transcode_topic_whitelist**: List of regular expressions ([ECMAScript grammar](https://en.cppreference.com/w/cpp/regex/ecmascript)) of topic names delivered as data over remote access instead of being transcoded to video. Use this for image topics whose pixel values must not pass through lossy video, such as compressed depth maps. Defaults to `[".*/compressedDepth"]`, matching the `compressed_depth_image_transport` `/compressedDepth` suffix.
