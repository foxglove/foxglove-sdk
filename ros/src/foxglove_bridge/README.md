...
- **topic_allowlist**: List of regular expressions ([ECMAScript grammar](https://en.cppreference.com/w/cpp/regex/ecmascript)) of allowed topic names. Defaults to `[".*"]`.
- **service_allowlist**: List of regular expressions ([ECMAScript grammar](https://en.cppreference.com/w/cpp/regex/ecmascript)) of allowed service names. Defaults to `[".*"]`.
- **param_allowlist**: List of regular expressions ([ECMAScript grammar](https://en.cppreference.com/w/cpp/regex/ecmascript)) of allowed parameter names. Defaults to `[".*"]`.
- **client_topic_allowlist**: List of regular expressions ([ECMAScript grammar](https://en.cppreference.com/w/cpp/regex/ecmascript)) of allowed client-published topic names. Defaults to `[".*"]`.
...
> **Note:** The `*_allowlist` parameters above were previously named `*_whitelist` (`topic_whitelist`, `service_whitelist`, `param_whitelist`, `client_topic_whitelist`, `best_effort_qos_topic_whitelist`). The old names are still accepted as deprecated aliases — both as node parameters (setting a `*_whitelist` parameter logs a warning) and as arguments to the provided launch file — but prefer the `*_allowlist` names. If both a `*_allowlist` and its `*_whitelist` alias are set, the `*_allowlist` value wins.
>
