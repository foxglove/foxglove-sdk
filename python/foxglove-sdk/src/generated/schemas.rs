...
///     - `yuv422`,  `uyvy`:
///       - 8-bit `Y'UV <https://en.wikipedia.org/wiki/Y%E2%80%B2UV>`__.
...
///       - `step` must be greater than or equal to `width`.
///     - `mono16`, `16UC1`:
///       - 16-bit abstract per-pixel values. Rendering of these values is controlled in `Image panel color mode settings <https://docs.foxglove.dev/docs/visualization/panels/image#general>`__.
///       - `step` must be greater than or equal to `width` * 2.
///     
///
