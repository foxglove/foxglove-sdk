...
pub fn register_submodule(parent_module: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    module.add_class::<Grid3DChannel>()?;
...
/// A channel for logging :py:class:`foxglove.schemas.Grid3D` messages.
#[pyclass(module = "foxglove.channels")]
struct Grid3DChannel(Channel<foxglove::schemas::Grid3D>);
...
impl Grid3DChannel {
    ...
    /// Log a :py:class:`foxglove.schemas.Grid3D` message to the channel.
    ///
    /// :param msg: The message to log.
    /// :param log_time: The log time is the time, as nanoseconds from the unix epoch, that the
    ///     message was recorded. Usually this is the time log() is called. If omitted, the
    ///     current time is used.
    #[pyo3(signature = (msg, *, log_time=None))]
    fn log(&self, msg: &schemas::Grid3D, log_time: Option<u64>) {
        let metadata = PartialMetadata { log_time };
        self.0.log_with_meta(&msg.0, metadata);
    }

    fn __repr__(&self) -> String {
        format!("Grid3DChannel(id={}, topic='{}')", self.id(), self.topic()).to_string()
    }
}
