...
Classes for loading Foxglove dataset episodes as a `Ray Data
<https://docs.ray.io/en/latest/data/data.html>`__ dataset, downloading them in parallel
...
Each row is a dict ``{"episode": {"episode_id": str, "mcap": bytes}}``. Decode the MCAP
bytes downstream, for example with ``ds.flat_map(...)``.
