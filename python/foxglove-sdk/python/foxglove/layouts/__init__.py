...
class StateTransitionsSeries(_BaseModel):
    ...
    """
    For a value path that resolves to an array of primitives (numbers, strings, or booleans): how to expand the array into series. "none" (the default) preserves the path's existing behavior: a [:] slice stays in one row, while a bare array path is not expanded. "byIndex" renders one sub-series per array index, using the index as the sub-series label. Arrays of label/value objects are always grouped by label.
    """
...
class PlotSeriesBase(_BaseModel):
    ...
    """
    How the line connects data points. ``linear`` draws a straight line between points; ``step`` holds each value until the next point, then steps to it.
    """
...
@dataclass(kw_only=True)
class BaseRendererThreeDimensionalMapReferenceMeasurement(_BaseModel):
    """
    3D map tile measurement used for altitude alignment.
    """
    ...
    altitude: float
    """
    Elevation in meters sampled from downloaded 3D map tiles.
    """
...
class BaseRendererThreeDimensionalMapLayerSettings(BaseRendererBaseLayerSettings):
    ...
    altitude_mode: Literal["auto", "location", "map-sampling", "fixed"] | None = None
    """
    How to align 3D map altitude. ``auto`` prefers a usable location fix altitude, then map sampling, then a fixed reference. ``location`` uses the location fix altitude. ``map-sampling`` samples the 3D map even when the location fix has an altitude. ``fixed`` anchors a fixed reference altitude to the Location frame.
    """

    reference_measurement: (
        BaseRendererThreeDimensionalMapReferenceMeasurement | None
    ) = None
    ...
    def _to_dict(self) -> dict[str, Any]:
        return super()._to_dict() | {
            ...
            "referenceMeasurement": self.reference_measurement,
...
class BaseRendererTransforms(_BaseModel):
    ...
    enable_transform_sync: bool | None = None
    """
    For timestamp-based synchronization, require each non-root frame referenced by a synced topic to have a transform to its immediate parent at the synced message timestamp. Ancestor transforms are not checked. Root frames do not require a transform.
    """
    ...
    def _to_dict(self) -> dict[str, Any]:
        return {
            ...
            "enableTransformSync": self.enable_transform_sync,
            "drawBehind": self.draw_behind,
        }
...
class BaseRendererConfig(_BaseModel):
    ...
    location_fix_frame: Literal["display"] | None = None
    """
    Frame source for the selected location fix. ``display`` uses the display frame; when unset, the frame from the location fix message is used.
    """
    ...
    def _to_dict(self) -> dict[str, Any]:
        return {
            ...
            "locationFixFrame": self.location_fix_frame,
