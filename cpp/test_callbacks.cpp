...
int main() {
    ...
    options.callbacks.onClientDisconnect = []() {
        disconnect_count++;
        std::cout << "Client disconnected! Total disconnections: " << disconnect_count.load() << std::endl;
    };
    
    // Create and start server
    auto server_result = foxglove::WebSocketServer::create(std::move(options));
    if (!server_result.has_value()) {
        std::cerr << "Failed to create server: " << static_cast<int>(server_result.error()) << std::endl;
        return 1;
    }
    ...
    std::cout << "You can use the Foxglove app or any WebSocket client to connect to ws://localhost:8765" << std::endl;
