...
int main() {
  ...
  for (int i = 0; i < 10; i++) {
    ...
    ch1.log(reinterpret_cast<const unsigned char*>(json_val.c_str()), json_val.size());

    // MessagePack
    std::vector<std::uint8_t> msgpack_val = json::to_msgpack(value);
    ch2.log(reinterpret_cast<const unsigned char*>(msgpack_val.data()), msgpack_val.size());
