...
class CameraCapture {
public:
    CameraCapture(const std::string& camera_id) : camera_id_(camera_id) {}
    bool connect() {
        try {
            int cam_id = 0;
            try {
...
int main(int argc, char** argv) {
    ...
    if (!server_result.has_value()) {
        std::cerr << "Failed to create server: " << foxglove::strerror(server_result.error()) << std::endl;
        return 1;
    }
    ...
    if (!image_channel_result.has_value()) {
        std::cerr << "Failed to create image channel: " << foxglove::strerror(image_channel_result.error()) << std::endl;
        return 1;
    }
