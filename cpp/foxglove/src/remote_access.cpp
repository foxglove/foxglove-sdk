...
FoxgloveResult<RemoteAccessGateway> RemoteAccessGateway::create(
  ...
  // Services — release ownership to C
  std::vector<foxglove_service*> c_services;
  c_services.reserve(options.services.size());
  for (auto& service : options.services) {
    c_services.push_back(service.release());
  }
  c_options.services = c_services.data();
  c_options.services_count = c_services.size();
