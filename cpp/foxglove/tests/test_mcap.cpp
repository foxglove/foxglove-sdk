...
class FileCleanup {
  public:
    FileCleanup(const std::string& path) : _path(path) {}
    ~FileCleanup() {
...
TEST_CASE("specify profile and library") {
  FileCleanup cleanup("test.mcap");
  ...
  REQUIRE(content.find("test_profile") != std::string::npos);
  REQUIRE(content.find("test_library") != std::string::npos);
}
