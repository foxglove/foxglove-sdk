...
/// @brief A 2D grid of occupancy data
struct OccupancyGrid {
  ...
  /// @brief Occupancy grid cell data, in row-major (y-major) order. Each byte represents a single
  /// cell's occupancy value as a signed 8-bit integer: 0 represents a free cell, 100 represents an
  /// occupied cell, and -1 (255 as unsigned) represents an unknown cell. Values between 0 and 100
  /// represent the probability that the cell is occupied.
  std::vector<std::byte> data;

  /// @brief Encoded the OccupancyGrid as protobuf to the provided buffer.
  ///
  /// On success, writes the serialized length to *encoded_len.
  /// If the provided buffer has insufficient capacity, writes the required capacity to *encoded_len
  /// and returns FoxgloveError::BufferTooShort.
  ...
  /// @brief Get the OccupancyGrid schema.
  ///
  /// The schema data returned is statically allocated.
  static Schema schema();
};
...
/// @brief A channel for logging OccupancyGrid messages to a topic.
///
/// @note While channels are fully thread-safe, the OccupancyGrid struct is not thread-safe.
/// Avoid modifying it concurrently or during a log operation.
class OccupancyGridChannel {
public:
  /// @brief Create a new channel.
  ///
  /// @param topic The topic name. You should choose a unique topic name per channel for
  ...
  /// @param context The context which associates logs to a sink. If omitted, the default context is
  /// used.
  static FoxgloveResult<OccupancyGridChannel> create(
    const std::string_view& topic, const Context& context = Context()
  );

  /// @brief Log a message to the channel.
  ///
  /// @param msg The OccupancyGrid message to log.
  /// @param log_time The timestamp of the message, as nanoseconds since epoch. If omitted, the
  /// current time is used.
  /// @param sink_id The ID of the sink to log to. If omitted, the message is logged to all sinks.
  FoxgloveError log(
    const OccupancyGrid& msg, std::optional<uint64_t> log_time = std::nullopt,
    std::optional<uint64_t> sink_id = std::nullopt
  ) noexcept;
  ...
  OccupancyGridChannel(const OccupancyGridChannel& other) noexcept = delete;
  OccupancyGridChannel& operator=(const OccupancyGridChannel& other) noexcept = delete;
  /// @brief Default move constructor.
  OccupancyGridChannel(OccupancyGridChannel&& other) noexcept = default;
  /// @brief Default move assignment.
  OccupancyGridChannel& operator=(OccupancyGridChannel&& other) noexcept = default;
  /// @brief Default destructor.
  ~OccupancyGridChannel() = default;

private:
  explicit OccupancyGridChannel(ChannelUniquePtr&& channel)
      : impl_(std::move(channel)) {}
