...
/// @brief A discrete event that occurred at a specific time or over a time range
struct Event {
  /// @brief Event start time
  std::optional<Timestamp> start_time;

  /// @brief Event end time. Omit for point events.
  std::optional<Timestamp> end_time;

  /// @brief Display name for the event
  std::string label;

  /// @brief Color used to visualize the event
  std::optional<Color> color;
