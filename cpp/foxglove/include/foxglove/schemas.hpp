...
struct EventProperty {
  ...
  enum class EventPropertyType : uint8_t {
    /// @brief Single-line text
    TEXT = 0,
    ...
    /// @brief Numeric value
    NUMBER = 3,
    /// @brief Single choice from options
    SINGLE_SELECT = 4,
    /// @brief Multiple choices from options
    MULTI_SELECT = 5,
  };
  /// @brief Property name
  std::string key;
  ...
  /// @brief String-encoded value. Interpretation depends on type.
  std::string value;

  /// @brief Encoded the EventProperty as protobuf to the provided buffer.
  ///
  /// On success, writes the serialized length to *encoded_len.
  /// If the provided buffer has insufficient capacity, writes the required capacity to *encoded_len
  /// and returns FoxgloveError::BufferTooShort.
  ...
  /// @brief Get the EventProperty schema.
  ///
  /// The schema data returned is statically allocated.
  static Schema schema();
};
...
/// @brief A channel for logging EventProperty messages to a topic.
///
/// @note While channels are fully thread-safe, the EventProperty struct is not thread-safe.
/// Avoid modifying it concurrently or during a log operation.
class EventPropertyChannel {
public:
  /// @brief Create a new channel.
  ///
  /// @param topic The topic name. You should choose a unique topic name per channel for
  ...
  /// @param context The context which associates logs to a sink. If omitted, the default context is
  /// used.
  static FoxgloveResult<EventPropertyChannel> create(
    const std::string_view& topic, const Context& context = Context()
  );

  /// @brief Log a message to the channel.
  ///
  /// @param msg The EventProperty message to log.
  /// @param log_time The timestamp of the message, as nanoseconds since epoch. If omitted, the
  /// current time is used.
  /// @param sink_id The ID of the sink to log to. If omitted, the message is logged to all sinks.
  FoxgloveError log(
    const EventProperty& msg, std::optional<uint64_t> log_time = std::nullopt,
    std::optional<uint64_t> sink_id = std::nullopt
  ) noexcept;
  ...
  EventPropertyChannel(const EventPropertyChannel& other) noexcept = delete;
  EventPropertyChannel& operator=(const EventPropertyChannel& other) noexcept = delete;
  /// @brief Default move constructor.
  EventPropertyChannel(EventPropertyChannel&& other) noexcept = default;
  /// @brief Default move assignment.
  EventPropertyChannel& operator=(EventPropertyChannel&& other) noexcept = default;
  /// @brief Default destructor.
  ~EventPropertyChannel() = default;
