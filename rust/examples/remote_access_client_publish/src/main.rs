//! Remote access example with client publish support: streams synthetic camera
//! feeds through the Foxglove gateway while accepting published messages from
//! connected clients. Produces a "test card" stream designed to make network
//! degradation (latency, packet loss, low bandwidth) visually obvious, plus a
//! simple scrolling gradient for a secondary feed.
