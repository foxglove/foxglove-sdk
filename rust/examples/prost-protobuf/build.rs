...
// This is a simplfied version of foxglove_proto_gen,
// which we use internally to generate prost types for the Foxglove schemas in the sdk.
// It uses the prost_build crate to generate the protobuf types, Foxglove Encode impls (including channel schemas).
...
pub fn generate_protos(proto_path: &Path, out_dir: &Path) -> anyhow::Result<()> {
    ...
    config.extern_path(".google.protobuf.Duration", "foxglove::schemas::Duration");
    config.extern_path(".google.protobuf.Timestamp", "foxglove::schemas::Timestamp");
    ...
    generate_crate(out_dir, &fds).context("Failed to generate crate")?;
...
fn generate_crate(out_dir: &Path, fds: &FileDescriptorSet) -> anyhow::Result<()> {
    // Append .rs for the crate "exports" module (instead of mod.rs)
    let mut mod_file = out_dir.to_path_buf();
    mod_file.set_extension("rs");
    let mut module = File::create(mod_file).context("Failed to create crate module")?;

    let mut result = writeln!(module, "// This file is @generated");
    result = result.and(writeln!(module, "mod descriptors;"));
    result = result.and(writeln!(module, "mod impls;"));
    result.context("Failed to write crate module")?;

    for fd in &fds.file {
        if let Some(name) = fd.name.as_ref().and_then(|n| n.strip_suffix(".proto")) {
            let kebab_name = camel_case_to_kebab_case(name);
            writeln!(module, "mod {kebab_name};").context("Failed to write crate module")?;
            writeln!(module, "pub use {kebab_name}::*;").context("Failed to write crate module")?;
        }
    }
...
/// Generates base64-encoded file descriptor sets for each message.
fn generate_descriptors(out_dir: &Path, fds: &FileDescriptorSet) -> anyhow::Result<()> {
    ...
    for fd in &fds.file {
        for message in &fd.message_type {
            if let Some(name) = message.name.as_ref() {
                ...
                let mut descr_file =
                    File::create(&path).context("Failed to create descriptor file")?;
    ...
    writeln!(module, "// This file is @generated").context("Failed to write descriptors.rs")?;
...
fn generate_impls(out_dir: &Path, fds: &FileDescriptorSet) -> anyhow::Result<()> {
    ...
    let mut result = writeln!(module, "// This file is @generated");
    result = result.and(writeln!(module, "use foxglove::{{Encode, Schema}};"));
    result = result.and(writeln!(module, "use bytes::BufMut;"));
    result.context("Failed to write impls.rs")?;

    for fd in &fds.file {
        for message in &fd.message_type {
            let Some(schema_name) = message.name.as_ref() else {
                continue;
            };
            let Some(package_name) = fd.package.as_ref() else {
                continue;
            };
            let descriptor_name = camel_case_to_constant_case(schema_name);
            writeln!(
                module,
                "\nimpl Encode for super::{schema_name} {{
    ...
    fn get_schema() -> Option<Schema> {{
        Some(Schema::new(
            ...
            \"protobuf\",
            super::descriptors::{descriptor_name},
        ))
    }}
...
fn main() -> anyhow::Result<()> {
    generate_protos(Path::new("protos"), Path::new("src/protos"))?;
