...
//!
//! Usage:
//!   # Start the stack (see docker-compose.netem-livekit.yml for full env vars)
//!   COMPOSE="docker compose -f docker-compose.yaml -f docker-compose.netem.yml -f docker-compose.netem-livekit.yml"
//!   $COMPOSE --profile perlink up -d --wait
//!
//!   # Run both sides concurrently
//!   $COMPOSE exec -T gateway-runner cargo test -p remote_access_tests -- --ignored perlink_docker_gateway --nocapture &
//!   $COMPOSE exec -T viewer-runner cargo test -p remote_access_tests -- --ignored perlink_docker_viewer --nocapture &
//!   wait
