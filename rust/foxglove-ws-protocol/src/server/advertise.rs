...
use crate::schema::{decode_schema_data, encode_schema_data, is_schema_required, Schema};
...
impl<'a> Channel<'a> {
    ...
    pub fn schema(&self) -> Result<Vec<u8>, crate::schema::DecodeError> {
        if let Some(schema_encoding) = self.schema_encoding.as_ref() {
            decode_schema_data(schema_encoding, &self.schema)
        } else {
            Err(crate::schema::DecodeError::MissingSchema)
        }
    }
...
impl<'a> TryFrom<Channel<'a>> for Schema<'a> {
    type Error = crate::schema::DecodeError;

    fn try_from(value: Channel<'a>) -> Result<Self, crate::schema::DecodeError> {
...
impl<'a> ChannelBuilder<'a> {
    ...
    pub fn build(self) -> Result<Channel<'a>, crate::schema::EncodeError> {
        match self.schema {
            None => {
                if is_schema_required(&self.encoding) {
                    Err(crate::schema::EncodeError::MissingSchema)
                } else {
                    Ok(Channel {
                        id: self.id,
                        topic: self.topic,
            ...
            Some(schema) => Ok(Channel {
                ...
                topic: self.topic,
                encoding: self.encoding,
                schema: encode_schema_data(&schema.encoding, schema.data)?,
