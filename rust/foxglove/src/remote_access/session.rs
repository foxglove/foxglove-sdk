...
impl RemoteAccessSession {
    ...
    fn handle_client_message_data(&self, participant: &Arc<Participant>, msg: client::MessageData) {
        ...
        let descriptor = {
            let state = self.state.read();
            let Some(descriptor) =
                state.get_client_channel(participant.participant_id(), channel_id)
            else {
        ...
        // Call the listener after releasing the state read lock.
        let client = Client::new(
            participant.client_id(),
            participant.participant_id().clone(),
        );
