...
pub(crate) mod transcode {
    ...
    pub(crate) fn transcode_point_cloud_message(
        ...
        // The float64 check runs on the converted cloud, so ROS 2 FLOAT64 fields are
        // detected the same way as native ones. Empty clouds are excluded: they fold to
        // lossless regardless of fields, and their output is header-sized, so there is
        // no size reduction to lose.
...
mod transcode_tests {
    ...
    fn test_transcodes_cdr_point_cloud2_to_compressed_point_cloud() {
        ...
        let transcoded = super::transcode::transcode_point_cloud_message(
            &encoded,
            PointCloudInputSchema::Ros2PointCloud2,
        ...
        .unwrap();
        let compressed =
            <crate::messages::CompressedPointCloud as Decode>::decode(transcoded.data.as_ref())
                .unwrap();
