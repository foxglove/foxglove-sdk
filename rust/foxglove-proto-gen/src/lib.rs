...
fn generate_types(out_dir: &Path, fds: &FileDescriptorSet) -> anyhow::Result<()> {
    let mut module = File::create(out_dir.join("types.rs")).context("Failed to create types.rs")?;
    writeln!(module, "// This file is @generated by foxglove-proto-gen\n")?;

    writeln!(module, "#[repr(u8)]")?;
    writeln!(module, "pub enum BuiltinSchema {{")?;
    for fd in &fds.file {
        let Some(mut name) = fd
            .name
            .as_ref()
...
pub fn generate_protos(proto_path: &Path, out_dir: &Path) -> anyhow::Result<()> {
    ...
    generate_types(out_dir, &fds).context("Failed to generate types")?;
