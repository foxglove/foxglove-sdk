...
function cppToC(schema: FoxgloveMessageSchema, copyTypes: Set<string>): string[] {
  return schema.fields.map((field) => {
    ...
    switch (field.type.type) {
      ...
      case "enum":
        if (field.optional) {
          return `dest.${dstName} = src.${srcName} ? static_cast<foxglove_${toSnakeCase(field.type.enum.name)}>(*src.${srcName}) : static_cast<foxglove_${toSnakeCase(field.type.enum.name)}>(0);`;
        }
