...
function assertValidBindgen(
  bindgen: TomlTable,
): asserts bindgen is TomlTable & { export: { rename: TomlTable } } {
  if (
...
function addSchemaRenames(
  bindgenToml: { export: { rename: TomlTable } },
  schemas: readonly FoxgloveMessageSchema[],
  enums: readonly FoxgloveEnumSchema[],
) {
...
/** Derive an include guard from a header filename, e.g. "foxglove-c.h" → "FOXGLOVE_C_H". */
function includeGuardFromFilename(filename: string): string {
  return filename.replace(/[-./]/g, "_").toUpperCase();
}

/**
 * Builds the content of a cbindgen.toml config file, based on a manually-written prelude
 * and export renames for generated schema types.
 *
 * @param outputFilename - The header filename (e.g. "foxglove-c.h"), used to derive the include guard.
 */
export async function generateBindgenConfig(
  ...
  bindgenToml.include_guard = includeGuardFromFilename(outputFilename);

  addSchemaRenames(bindgenToml, schemas, enums);

  return BINDGEN_COMMENT + "\n" + stringifyToml(bindgenToml);
}
