...
- \`yuv422\` or  \`uyvy\`:
  ...
  - Pixel channel values are represented as unsigned 8-bit values.
...
- \`bayer_rggb8\`, \`bayer_bggr8\`, \`bayer_rggb8\`, \`bayer_gbrg8\`, or \`bayer_grgb8\`:
  - Pixel colors are decomposed into Red, Blue and Green channels.
  - Pixel channel values are represented as unsigned 8-bit values, and serialized in a 2x2 bayer filter pattern.
...
- \`mono8\` or \`8UC1\`:
  - Pixel brightness is represented as unsigned 8-bit values.
  - \`step\` must be greater than or equal to \`width\`.
- \`mono16\` or \`16UC1\`:
  - 16-bit little-endian abstract per-pixel values. Rendering of these values is controlled in [Image panel color mode settings](https://docs.foxglove.dev/docs/visualization/panels/image#general).
  - \`step\` must be greater than or equal to \`width\` * 2.
`,
    },
  ],
