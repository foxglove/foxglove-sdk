    ...
    .flatMap((field) => {
      ...
      switch (field.type.type) {
        ...
        case "enum":
          ...
          break;
        case "nested":
          fieldType = `*const ${field.type.schema.name}`;
          break;
      }
      let lines: string[] = [comment];
      if (typeof field.array === "number") {
        lines.push(`pub ${escapeId(field.name)}: [${fieldType}; ${field.array}],`);
        if (fieldHasLen) {
          lines.push(`pub ${escapeId(field.name)}_len: [usize; ${field.array}],`);
        }
      } else if (field.array === true) {
        lines.push(`pub ${escapeId(field.name)}: *const ${fieldType},`);
        if (fieldHasLen) {
          lines.push(`pub ${escapeId(field.name)}_len: *const usize,`);
        }
        lines.push(`pub ${escapeId(field.name)}_count: usize,`);
      } else {
        lines.push(`pub ${escapeId(field.name)}: ${fieldType},`);
        if (fieldHasLen) {
          lines.push(`pub ${escapeId(field.name)}_len: usize,`);
        }
      }
      return lines.join("\n");
    })
          ...
          switch (field.type.type) {
            case "primitive":
              if (field.type.name === "string") {
                ...
                return `${dstName}: Bytes::copy_from_slice(unsafe { std::slice::from_raw_parts(msg.${srcName}, msg.${field.name}_len) })`;
